  ...
  const getProducts = async (categoryId, page) => {
    ...
    try {
      ...
      if (categoryId && categoryId !== 'default') {
        const response = await axios.get(`${apiUrl}/products?categoryId=${categoryId}&page=${page}&showAll=true&limit=100`, {
          headers: {
        ...
        const response = await axios.get(`${apiUrl}/products?page=${page}&showAll=true&limit=100`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        })
