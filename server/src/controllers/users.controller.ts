...
const getMe = async (req: Request, res: Response) => {
    ...
    const { statusCode, response } = await service.getMe(id);
    res.status(statusCode).json(response);
}
const updateMe = async (req: Request, res: Response) => {
    const id = req.body.userId
    delete req.body.userId;
    const { statusCode, response } = await service.updateMe(id, req.body);
...
export default { getAll, getById, create, getMe, updateMe, update, remove, login, register };